...
switch -exact -- $ID_FACTOR {
  ...
  default {
    puts "Error: ID_FACTOR($ID_FACTOR) not supported"
    quit
  }
}
...
cd build
open_project "${module}_DD${DD}_${op_name}_${type_name}_${id_name}.hls_prj"
set_top "${module}"
add_files ../core.cpp -cflags "-Wmissing-field-initializers -DDD=${DD} -DOP_CODE=${OP_CODE} -DDATATYPE=${DATATYPE} -DID_FACTOR=${ID_FACTOR}" -csimflags "-Wmissing-field-initializers  -DOP_CODE=${OP_CODE} -DDATATYPE=${DATATYPE} -DID_FACTOR=${ID_FACTOR}"
add_files ../../raw_dep.cpp -cflags "-Wmissing-field-initializers -DDD=${DD} -DOP_CODE=${OP_CODE} -DDATATYPE=${DATATYPE} -DID_FACTOR=${ID_FACTOR}" -csimflags "-Wmissing-field-initializers  -DOP_CODE=${OP_CODE} -DDATATYPE=${DATATYPE} -DID_FACTOR=${ID_FACTOR}"
add_files -tb ../../test/test.cpp -cflags "-Wno-unknown-pragmas -Wmissing-field-initializers -DOP_CODE=${OP_CODE} -DDATATYPE=${DATATYPE} -DID_FACTOR=${ID_FACTOR}" -csimflags "-std=c++14 -fexceptions -Wno-unknown-pragmas -Wmissing-field-initializers -DOP_CODE=${OP_CODE} -DDATATYPE=${DATATYPE} -DID_FACTOR=${ID_FACTOR}"
